...
const DIMENSIONS = [
  ...
  },
  {
    ...
    color: "#8B5CF6",
    descriptions: [
      "Standard OData, no custom logic",
      "Standard OData with light customisation",
      "Custom CDS view, single source",
      "Custom CDS with cross-module joins",
      "Side-by-side BTP extension + custom CDS",
    ],
  },
  {
    id: "d3",
    ...
    color: "#F59E0B",
    descriptions: [
      "Clean-core extension (SAP-approved)",
      "Key-user extension via Fiori adaptation",
      "Developer extension with public extension points",
      "Modification touches standard objects (isolated)",
      "Direct modification of standard SAP objects",
    ],
  },
  {
    id: "d4",
...
const TREE_QUESTIONS = [
  {
    id: "q1",
    label: "Q1",
    question: "Does a standard SAP Fiori application already cover this requirement?",
    theory: "TOE — Environment: vendor-supplied alternative set",
    yes: "outcome-standard",
    no: "q2",
  },
  {
    id: "q2",
    label: "Q2",
    question: "Can SAP configuration options (without custom code) close the gap?",
    theory: "TOE — Organisation: minimum change capacity",
    yes: "outcome-configure",
    no: "q3",
  },
  {
    id: "q3",
    label: "Q3",
    question: "Does this requirement represent a genuine competitive advantage — or is it a legacy habit?",
    theory: "UET — Executive cognitive base",
    yes: "q4",
    no: "outcome-developer",
  },
  {
    id: "q4",
    label: "Q4",
    question: "Is the Pillar 1 composite score below the RED threshold (≤ 70)?",
    theory: "Pillar 1 integration gate",
    yes: "q5",
    no: "outcome-keyuser",
  },
  {
    id: "q5",
    label: "Q5",
    question: "Is a clean-core key-user extension technically feasible for this requirement?",
    theory: "TOE — Technology trajectory + Environment: SAP clean-core mandate",
    yes: "outcome-keyuser",
    no: "outcome-custom",
  },
];
...
const OUTCOMES = {
  "outcome-standard": {
    label: "Standard Fiori",
    color: "#10B981",
    bg: "#052e16",
    border: "#166534",
    description: "An out-of-the-box SAP Fiori application satisfies this requirement. No development required.",
    risk: "Lowest",
  },
  "outcome-configure": {
    label: "Configure",
    ...
    bg: "#0c1a2e",
    border: "#1e3a5f",
    description: "SAP-delivered configuration options close the gap. No custom code required.",
    risk: "Low",
  },
  "outcome-developer": {
    ...
    bg: "#1c1000",
    border: "#78350f",
    description: "A lightweight, clean-core developer extension. Justified by technical need, not competitive advantage.",
    risk: "Moderate",
  },
  "outcome-keyuser": {
    ...
    bg: "#1c1000",
    border: "#78350f",
    description: "Low-code adaptation via SAP Fiori adaptation tools. Clean-core compliant.",
    risk: "Moderate",
  },
  "outcome-custom": {
    label: "Custom UI5",
    ...
    bg: "#1c0000",
    border: "#7f1d1d",
    description: "Custom UI5 development is justified. Requires full governance approval and TCO sign-off.",
    risk: "Highest",
  },
};
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
        <div>
          <span style={{
            ...
            color: dim.color, letterSpacing: 2,
            background: dim.color + "20",
            padding: "2px 8px", borderRadius: 4,
          }}>
            {dim.code} · {Math.round(dim.weight * 100)}% weight
          </span>
          <div style={{ color: "#E2E8F0", fontWeight: 600, fontSize: 15, marginTop: 6 }}>
            {dim.label}
          </div>
        </div>
        <div style={{
          ...
          color: value > 0 ? dim.color : "#1E2D45",
...
function ScreenInput({ onNext }) {
  ...
  const composite = DIMENSIONS.reduce((sum, d) => {
    const raw = scores[d.id] || 0;
    return sum + raw * 20 * d.weight;
  }, 0);

  const allFilled =
    form.description && form.businessUnit && form.devDays &&
    ...
    <div style={{ display: "flex", gap: 24, alignItems: "flex-start" }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ color: "#64748B", fontSize: 14, marginTop: 6 }}>Enter the customisation request details and rate each governance dimension.</p>
        </div>

        {/* Request info */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
          {[
            { label: "Request ID", key: "requestId", placeholder: "REQ-001" },
            { label: "Business Unit", key: "businessUnit", placeholder: "Finance, SCM, HR..." },
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <textarea
            ...
            placeholder="Describe what the business unit is requesting..."
            rows={3}
            style={{
              width: "100%", background: "#111827",
              border: "1px solid #1E2D45", borderRadius: 8,
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <input
            type="number"
            value={form.devDays}
            onChange={(e) => setForm({ ...form, devDays: e.target.value })}
            placeholder="e.g. 10"
            style={{
              width: 160, background: "#111827",
              border: "1px solid #1E2D45", borderRadius: 8,
              padding: "10px 14px", color: "#E2E8F0",
              fontSize: 14, outline: "none",
              fontFamily: "JetBrains Mono, monospace",
        ...
        {DIMENSIONS.map((dim) => (
          <DimensionSlider
            ...
            dim={dim}
            value={scores[dim.id]}
            onChange={(v) => setScores({ ...scores, [dim.id]: v })}
          />
        ))}

        <button
          onClick={() => allFilled && onNext({ form, scores, composite })}
          disabled={!allFilled}
          style={{
            width: "100%", padding: "14px 0",
            background: allFilled ? "#3B82F6" : "#1E2D45",
      ...
      {/* Right — live preview */}
      <div style={{
        width: 220, position: "sticky", top: 24,
...
function ScreenPillar1({ data, onNext, onBack }) {
  const { form, scores, composite } = data;
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ color: "#E2E8F0", fontSize: 16, lineHeight: 1.6 }}>
            {verdictText[verdict]}
          </div>
        </div>
      </div>
          ...
          return (
            <div key={dim.id} style={{ marginBottom: 16 }}>
              ...
              <div style={{ color: "#64748B", fontSize: 11, marginTop: 4 }}>
                Score {raw}/5 — {dim.descriptions[raw - 1]}
              </div>
            </div>
          );
        })}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 24 }}>
        {[
          ...
          <div key={label} style={{
            background: highlight ? "#0c1a2e" : "#111827",
            border: `1px solid ${highlight ? "#1e3a5f" : "#1E2D45"}`,
            borderRadius: 12, padding: 20,
          }}>
            <div style={{ color: "#64748B", fontSize: 10, fontFamily: "JetBrains Mono, monospace", letterSpacing: 2, marginBottom: 8 }}>{label}</div>
            <div style={{ color: highlight ? "#3B82F6" : "#E2E8F0", fontSize: 22, fontWeight: 800, fontFamily: "JetBrains Mono, monospace" }}>{value}</div>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 24 }}>
        {TREE_QUESTIONS.map((q, i) => {
          const answered = history.find((h) => h.id === q.id);
          const isActive = current === q.id && !outcome;
          return (
            <div key={q.id} style={{ flex: 1, height: 4, borderRadius: 2, background: answered ? "#3B82F6" : isActive ? "#3B82F640" : "#1E2D45" }} />
          );
        })}
      </div>
        ...
        return (
          <div key={h.id} style={{
            ...
            <span style={{
              ...
              color: h.answer === "yes" ? "#10B981" : "#EF4444",
              background: h.answer === "yes" ? "#10B98120" : "#EF444420",
      ...
      {!outcome && currentQ && (
        <div style={{
          ...
          <div style={{ color: "#3B82F6", fontFamily: "JetBrains Mono, monospace", fontSize: 11, fontWeight: 700, letterSpacing: 2, marginBottom: 12 }}>
            {currentQ.label} · ACTIVE
          </div>
          <div style={{ color: "#E2E8F0", fontSize: 18, fontWeight: 600, lineHeight: 1.5, marginBottom: 8 }}>
            {currentQ.question}
          </div>
          <div style={{ color: "#64748B", fontSize: 12, fontStyle: "italic", marginBottom: 24 }}>
            {currentQ.theory}
          </div>
          <div style={{ display: "flex", gap: 12 }}>
      ...
      {outcomeData && (
        <div style={{
          background: outcomeData.bg, border: `1px solid ${outcomeData.border}`,
          borderRadius: 16, padding: 28, marginBottom: 16,
        }}>
          <div style={{ color: outcomeData.color, fontFamily: "JetBrains Mono, monospace", fontSize: 11, fontWeight: 700, letterSpacing: 2, marginBottom: 8 }}>
            OUTCOME DETERMINED
          </div>
          <div style={{ color: "#E2E8F0", fontSize: 22, fontWeight: 800, marginBottom: 8 }}>
            {outcomeData.label}
          </div>
          <div style={{ color: "#E2E8F0", fontSize: 15, lineHeight: 1.6, marginBottom: 12 }}>
...
function ScreenResult({ data, onRestart }) {
  const { form, scores, composite, verdict, outcomeData } = data;
  const buildCost = Number(form.devDays) * 1200;
  ...
  return (
    <div style={{
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {screens.map((s, i) => (
            <div key={s} style={{
              ...
              background: screen === i ? "#1E2D45" : "transparent",
              color: screen === i ? "#E2E8F0" : "#64748B",
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px" }}>
        {screen === 0 && (
          <ScreenInput onNext={(d) => { setSessionData(d); setScreen(1); }} />
        )}
        {screen === 1 && (
          <ScreenPillar1 data={sessionData} onNext={(d) => { setSessionData(d); setScreen(2); }} onBack={() => setScreen(0)} />
        )}
        {screen === 2 && (
          <ScreenPillar2 data={sessionData} onNext={(d) => { setSessionData(d); setScreen(3); }} onBack={() => setScreen(1)} />
        )}
        {screen === 3 && (
          <ScreenResult data={sessionData} onRestart={() => { setSessionData({}); setScreen(0); }} />
        )}
      </div>
    </div>
  );
